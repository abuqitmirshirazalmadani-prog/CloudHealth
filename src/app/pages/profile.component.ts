  ...
  template: `
    <div class="p-4 md:p-8 max-w-4xl mx-auto">
      <header class="mb-8 flex items-center justify-between">
        ...
        <button (click)="openEditProfileModal()" class="bg-slate-900 hover:bg-slate-800 text-white px-4 py-2 rounded-xl text-sm font-medium transition-colors cursor-pointer">
          Edit Profile
        </button>
      </header>
        ...
        @if (showEditProfileModal()) {
          <div class="fixed inset-0 bg-slate-900/50 flex items-center justify-center z-50 p-4">
            <div class="bg-white rounded-3xl p-6 w-full max-w-md shadow-xl">
              ...
              <div class="flex justify-end gap-3 mt-8">
                <button (click)="showEditProfileModal.set(false)" class="px-5 py-2.5 rounded-xl text-slate-600 font-medium hover:bg-slate-50 transition-colors cursor-pointer">Cancel</button>
                <button (click)="saveProfile()" [disabled]="!editProfileData.name" class="px-5 py-2.5 rounded-xl bg-emerald-600 text-white font-medium hover:bg-emerald-700 transition-colors disabled:opacity-50 cursor-pointer">Save</button>
              </div>
            </div>
          </div>
        }
...
export class ProfileComponent {
  ...
  async saveProfile() {
    const profile = this.dataService.userProfile();
    if (!profile || !this.editProfileData.name) return;

    await this.dataService.updateUserProfile(profile.id, { 
      name: this.editProfileData.name,
      height: this.editProfileData.height,
      weight: this.editProfileData.weight,
      bloodType: this.editProfileData.bloodType
    });
